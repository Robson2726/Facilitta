...
// Função para mapear status desktop -> mobile
...
app.get('/api/encomendas', checkDb, async (req, res) => {
  try {
    ...
    // Mapeia para formato esperado pelo mobile com formato de data/hora correto
    const encomendas = result.rows.map(row => {
      // CORREÇÃO: Forçar interpretação como horário local brasileiro
      const dataOriginal = new Date(row.data_recebimento);
      const dataEntregaOriginal = row.data_entrega ? new Date(row.data_entrega) : null;
      
      // Extrair apenas a data no formato YYYY-MM-DD sem conversão de timezone
      const dataRecebimentoStr = row.data_recebimento.toISOString().split('T')[0];
      const dataEntregaStr = row.data_entrega ? row.data_entrega.toISOString().split('T')[0] : null;
      ...
      return {
        ...
        hora_recebimento: dataOriginal.toTimeString().substring(0, 5),
        porteiro_nome: row.porteiro_nome || 'N/A',
        observacoes: row.observacoes,
        status: mapStatusToMobile(row.status_original),
        data_entrega: dataEntregaStr,
        hora_entrega: dataEntregaOriginal ? dataEntregaOriginal.toTimeString().substring(0, 5) : null,
