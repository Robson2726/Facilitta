...
export default function DashboardScreen() {
  ...
  const carregarDashboard = async () => {
    try {
      ...
      console.log('[Dashboard] Resposta completa da API:', result);
      console.log('[Dashboard] Debug info da API:', result.debug);

      if (result.success && Array.isArray(result.data)) {
        const encomendas = result.data;
        console.log('[Dashboard] ENCOMENDAS RECEBIDAS DO BACKEND:', encomendas.length);
        console.log('[Dashboard] DEBUG - Informações do backend:', result.debug);
        
        // Log das primeiras encomendas para verificar estrutura
        if (encomendas.length > 0) {
          console.log('[Dashboard] Primeiras 3 encomendas recebidas:');
          encomendas.slice(0, 3).forEach((e: any, index: number) => {
            console.log(`[Dashboard] Encomenda ${index + 1}:`, {
              id: e.id,
              morador: e.morador_nome,
              status: e.status,
        ...
        const totalPendentes = encomendas.length;
        
        // ...existing code for period calculations...
      } else {
