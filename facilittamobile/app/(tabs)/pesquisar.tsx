...
export default function PesquisarScreen() {
  ...
  const buscarEncomendas = async () => {
    try {
      ...
      console.log('[Pesquisar] Resposta da API:', result);
      console.log('[Pesquisar] Debug info:', result.debug);
      
      if (result.success && Array.isArray(result.data)) {
        console.log('[Pesquisar] TOTAL DE ENCOMENDAS RECEBIDAS:', result.data.length);
        console.log('[Pesquisar] DEBUG DO BACKEND:', result.debug);
  ...
  const calcularDiasPendentes = (dataRecebimento: string) => {
    try {
      ...
      // Criar data de recebimento no horário local
      const dataRecebida = new Date(ano, mes - 1, dia);
      
      // Zerar horas para comparar apenas datas
      const hojeData = new Date(hoje.getFullYear(), hoje.getMonth(), hoje.getDate());
      const recebidaData = new Date(dataRecebida.getFullYear(), dataRecebida.getMonth(), dataRecebida.getDate());
      
      // Calcular diferença em milissegundos e converter para dias
      const diffTime = hojeData.getTime() - recebidaData.getTime();
      const diffDays = Math.floor(diffTime / (1000 * 60 * 60 * 24));
      
      return Math.max(0, diffDays);
    } catch (error) {
      console.warn('Erro ao calcular dias pendentes:', error, 'Data:', dataRecebimento);
      return 0;
    }
  };
  ...
  const getTempoColor = (dataRecebimento: string) => {
    ...
    if (dias > 15) return { color: '#dc3545', fontWeight: '600' as const }; // Vermelho para crítico
    if (dias > 7) return { color: '#ffc107', fontWeight: '600' as const }; // Amarelo para antigo
    return { color: '#28a745', fontWeight: '500' as const }; // Verde para recente
  };
  ...
  const renderItem = ({ item }: { item: Encomenda }) => {
    ...
    // CORREÇÃO: Todas as encomendas são pendentes
    const isPendente = true;
    const diasPendentes = calcularDiasPendentes(item.data_recebimento);
        ...
        >
          ...
          <View style={styles.itemDetails}>
            <View style={styles.detailRow}>
              ...
              <Text style={styles.detailText}>{item.quantidade} encomenda{item.quantidade > 1 ? 's' : ''}</Text>
            </View>
            <View style={styles.detailRow}>
              <Ionicons name="timer" size={14} color="#666" />
              <Text style={[styles.detailText, getTempoColor(item.data_recebimento)]}>
                {diasPendentes === 0 ? 'Hoje' : diasPendentes === 1 ? '1 dia' : `${diasPendentes} dias`}
              </Text>
            </View>
          </View>
