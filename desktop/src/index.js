...
async function getPendingPackages() {
  ...
  try {
    ...
    // Corrigido: inclui morador_id, apartamento e bloco
    const result = await client.query(
      `SELECT E.id, E.morador_id, M.nome AS morador_nome, M.apartamento, M.bloco, U.nome_completo AS porteiro_nome, E.data_recebimento, E.quantidade, E.status, E.observacoes 
       FROM encomendas E
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('buscar-relatorio', async (event, filtros) => {
    ...
    try {
      ...
      return { success: false, message: 'Erro ao buscar relatório: ' + error.message };
    } finally {
      if (client) client.release();
    }
  });
